...
class Login:
    def __init__(self):
        ...
        self.image_file = PhotoImage(file="material/badminton_bg.png")
        # 加载图片文件，并将图片置于画布上
    ...
    def login(self, account, password):
        ...
        if account.strip() and password.strip():
            if self.db.prepare(f"select * from admin_user where username='{account}' and password='{password}'"):
                showinfo("提示", "登录成功")
    ...
    def register(self, account, password):
        ...
        # 判断信息是否输入完整
        if account.strip() and password.strip():
            if self.db.prepare(f"select * from admin_user where username='{account}'") == 0:
                self.db.prepare(f"insert into admin_user (username,password) values ('{account}','{password}')")
                self.db.update()
                showinfo("提示", "注册成功")
            else:
                showerror("错误", "该名称已被注册")
        else:
